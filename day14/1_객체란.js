...
-프로그래밍 객체
     ...
     -인기가 많은 이유: 생활 속의 모든 정의된(성질/행위)를 디지털화 할 수 있다.코드화
...
-예시
   자동차
     기아 K3 
         -성질: 색상, 타이어 , 의자 등등
         -행동: 전진,후진,파킹,잠금 등등
        나의 k3와 친구 K3 는 서로 고유한 성질/행동 가질 수 있다. 그러므로 다른 객체다.
...
//문법
    ...
    3.속성
        1. 속성내 데이터 수정: 객체변수.속성명 =새로운 데이터
        2. 속성과 데이터 추가: 객체변수.새로운속성명= 새로운 데이터
    ...
    -다양한 속성값과 객체의 중첩
        {
         속성명: 데이터(문자,숫자,불리언, undifined등)
         {속성명: [데이터,데이터,데이터]},
         속성명: function(){},
         속성명: {}
        }
        -실무 웹/앱 JSON형식의 데이터 활용 중요!!
             JSON : JacaScript Object Notation : js 객체문법
             {키:값, 키:값}
             {속성: 데이터}
             -속성/키/필드/성질 명
             -값
    ...
    -키워드
     . : 접근 연산자: 객체내 속성에 접근/호출 할 때 사용되는 연산자/특수문자
...
console.log(객체변수.직업);
//console.log(객체변수. address-city);  //속성명에 특수문자가 포함될 경우 오류 발생
console.log(객체변수["address-city"]) //속성명에 특수문자 있을경우. 접근연산자 말고 []연산자를 사용하자.


//3.속성내 데이터 수정
객체변수.age=50;
객체변수.money=300; //만일 없는 속성명에 데이터 대입할 경우 새로운 속성명과 데이터가 추가됨
console.log(객체변수);
...
console.log(객체변수)
객체변수.직업= undefined; //속성은 있고 속성내 데이터가 없을 때, undifined 또는 공백으로 처리.
console.log(객체변수)
