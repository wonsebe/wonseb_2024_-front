...
주간 영화 목록API
1.사이드메뉴에서 '주간/주말 박스오피스' 선택
2. 페이지 하단에 ' 4. 응답 예시 ' 에 	JSON 주소 클릭
...
4.해당 페이지에 나온 데이터 자로코드를 복사해서 JSON 파일 변수에 출력하시오.

5.순위 영화명 개봉일 누적관객수 순으로 HTML 테이블에 출력하시오.
 */
...
console.log(영화API.boxOfficeResult); //자료결과 정보, 주간 박스오피스 정보
    //영화API.boxOfficeResult :{ weeklyBoxOfficeList: 값 }
console.log(영화API.boxOfficeResult.weeklyBoxOfficeList); //주간 박스오피스의 영화 정보의 목록
...
//2. HTML 출력 함수 : 실행조건 : 1. JS 열렸을 때 : 어디에 무엇을 출력 판단하기
moviePrint(); //함수 실행: Js가 실행될 때 함수 1번 실행해주기
function moviePrint(){ // 함수 정의/만들기
     ...
     //2. 무엇을
     let 무엇을 = ``;
     let 영화목록= 영화API.boxOfficeResult.weeklyBoxOfficeList; //변수를 선언시켜 코드 단순화
     for(let i=0; i<영화목록.length; i++){
        let 영화= 영화목록[i]; console.log(영화);

        //무엇을 에 반복해서 누적 HMTL 문자열 누적 대입
         //반복 1회마다 tr/행 1개
        무엇을 += `<tr>
                    <td> ${영화.rank}</td>
