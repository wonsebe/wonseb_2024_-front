...
let contentList = [ 
  ...
  { content : '신용카드발급',date : '2024-05-7' , color : 'pink' }
]
...
function calPrint(){
  ...
  // 2. 무엇을
    let html = `<div class="week sunday"> 일 </div>
                  <div class="week"> 월 </div> <div class="week"> 화 
                  </div> <div class="week"> 수 </div> <div class="week"> 목 
                  </div> <div class="week"> 금 </div> <div class="week"> 토 </div>`; 
    ...
    for( let day = 1 ; day <= endDay ; day++ ){
      ...
      let date3 = `${ year }-${ month}-${ (day<10? '0':'')+day}`;  console.log( date3 );
      let dayHtml= ``;
...
function monthChange( 매개변수 ){ 
  ...
  if( 매개변수 == '이전' ){ // 만일 월이 0 이면 12 을 대입하고 연도는 1차감
    month = Number(month) - 1 ; if( month == 0  ){ month = 12; year = year - 1 ; }
    // 아니고 만약에 다음 버튼을 클릭하면 월에서 1증가

  }else if( 매개변수 == '다음') {
    month = Number(month) + 1 ; if( month == 13 ){ month = 1 ; year = year + 1 ; }
  }
...
function add(index){
    ...
    let datee = document.querySelector('#index').value;

    let 할일추가 = {content : `${textInput}`, date : `${datee}`, color : `${color}`}

    contentList.push(할일추가);
