...
let hour= date.getHours() <10 ? "0"+(date.getMonth()+1): date.getMonth()+1;
...
function 추가함수(){
    ...
    let rr = document.querySelector('#rr').value;   console.log('rr');
    let date = new Date();  
    let 주문일= `${date.getFullYear()}-${date.getMonth()+1 <10 ? "0"+(date.getMonth()+1): date.getMonth()+1}-${date.getDate()} ${date.getHours() <10 ? "0"+(date.getHours()): date.getHours()}:${date.getMinutes()}`;  
     console.log(주문일);
