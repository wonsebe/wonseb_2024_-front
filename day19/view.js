...
게시물 상세페이지
    ...
    -무엇의 특정 게시물을 클릭 했는지 식별 필요하다.
     ...
     2. JS 변수 저장 x, [페이지가 전환되면  JS 도 초기화]
     3. *URL(주소)에 매개변수 넣어주는 방식
         ...
         URL ? 변수명 = 데이터 & 변수명 = 데이터
          ...
          <th><a href="view.html?no=7"> 변수명: no, 값: 7

          -JS에서 URL 매개변수를 호출하는 방법
           1. let urlParams =new URL(location.href ).searchParams;
           2. let value =urlParams.get('URl 변수명');
           3.검색된 결과를 html에 출력한다
*/

//new URL(검색할URL) : URL(주소)의 정보호출, location.href : 현재 URL
...
function board(){
    ...
    //출력
    boardBox.innerHTML=html; console.log('boardBox');
}
...
//3. 삭제 : 삭제 버튼을 클릭했을 때
 ...
 function _delete(){
    ...
    //-------------------------------------------------------------//
        ...
        localStorage.setItem('boardList', JSON.stringify(boardList)); // 삭제 최신화 
